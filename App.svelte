...
<script>
  ...
  var data1n = [...data_network.nodes];
  var data1l = [...data_network.links];
  var data2n = [...originlinks.nodes];
  var data2l = [...originlinks.links];
  var data3n = [...classlinks.nodes];
  var data3l = [...classlinks.links];
  ...
  $: {
    ...
    if (selected_network === "Only origin connections") {
      nodes = data2n;
      links = data2l;
    }
    if (selected_network === "") {
  ...
  Keep in mind though, a balanced team with multiple trait bonusses is the key to victory!  </h3>
      ...
      <div>
          ...
          <Graphic {width} {height} {scaleX} {scaleY}>
            ...
            {/each}
            <!-- Adding legend -->
            <select bind:value={selectedlegend} class="secondselect">
              <option value=""></option>
              <option value="origins">Origins</option>
              <option value="classes">Classes</option>
            </select>
            <rect class="legend"></rect>
            {#each legenddata as legend, i}
              <text class="title_properties" x={15} y={30}>LEGEND: Origins</text>
